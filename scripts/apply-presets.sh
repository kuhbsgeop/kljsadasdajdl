...
domain_trojan_port_for_index() {
  local index="$1"
  local base step
  base="${TROJAN_PORT:-9443}"
  step="${DOMAIN_PORT_STEP:-1}"
  [[ "$base" =~ ^[0-9]+$ ]] || { echo "TROJAN_PORT must be numeric: ${base}" >&2; exit 1; }
  [[ "$step" =~ ^[0-9]+$ ]] || { echo "DOMAIN_PORT_STEP must be numeric: ${step}" >&2; exit 1; }
  [ "$step" -ge 1 ] || { echo "DOMAIN_PORT_STEP must be >= 1." >&2; exit 1; }
  printf '%d' $((base + index * step))
}
...
build_trojan_domain_client_specs() {
  local tmp domain safe email password sub_id idx port
  tmp="$(mktemp)"
  sub_id="${ALL_NODES_SUB_ID:-$DEFAULT_SUB_ID}"
  idx=0
  : > "$tmp"
  ...
  domain_node_values | while IFS= read -r domain; do
    [ -n "$domain" ] || continue
    port="$(domain_trojan_port_for_index "$idx")"
    idx=$((idx + 1))
    safe="$(safe_node_label "$domain")"
    [ -n "$safe" ] || safe="domain"
    email="me-trojan-${safe}-${PRESET_CLIENT_SUFFIX}"
    ...
    jq -n \
      ...
      --argjson port "$port" \
      --arg subId "$sub_id" \
      '{domain:$domain,email:$email,password:$password,port:$port,subId:$subId}' >> "$tmp"
  done

  if [ ! -s "$tmp" ]; then
    email="me-trojan-${PRESET_CLIENT_SUFFIX}"
    jq -n \
      --arg domain "${SERVER_ADDR:-YOUR_SERVER_IP}" \
      --arg email "$email" \
      --arg password "$(rand_b64 24)" \
      --argjson port "$(domain_trojan_port_for_index 0)" \
      --arg subId "$sub_id" \
      '{domain:$domain,email:$email,password:$password,port:$port,subId:$subId}' >> "$tmp"
  fi
...
update_trojan_domain_clients() {
  ...
  resp="$(api_get "/panel/api/inbounds/list")"
  printf '%s\n' "$resp" > runtime/inbounds-before-trojan-domain-sync.json
  ...
  payload="$(
    jq -c \
      ...
      def obj:
        if type == "string" then (fromjson? // {}) else (. // {}) end;
      def generated_trojan:
        (((.email // "") | startswith("me-trojan-"))
        or ((.comment // "") | startswith("domain-node:"))
        or ((.comment // "") == "generated by 3xui-selfhost-kit"));
      .obj[]? as $inbound
      | select(($inbound.remark // "") == $remark)
      ...
      ($specs | map(
          ...
          | (old_by_email($spec.email)) as $old
          | {
              password: ($old.password // $spec.password),
              email: $spec.email,
              limitIp: ($old.limitIp // 0),
              totalGB: ($old.totalGB // 0),
              expiryTime: ($old.expiryTime // 0),
      ...
      | {
          ...
          total: ($inbound.total // 0),
          trafficReset: ($inbound.trafficReset // "never"),
          settings: ($settings | .clients = ((($oldClients // []) | map(select(generated_trojan | not))) + $domainClients)),
          streamSettings: ($inbound.streamSettings | obj),
          sniffing: ($inbound.sniffing | obj)
        }
      ' runtime/inbounds-before-trojan-domain-sync.json 2>/dev/null || true
  )"

  [ -n "$payload" ] || return 1
  id="$(printf '%s' "$payload" | jq -r '.id // empty')"
  [ -n "$id" ] || return 1
  file="runtime/trojan-domain-clients-update-${id}.json"
  printf '%s\n' "$payload" > "$file"
  echo "Syncing Trojan domain clients for ${remark}"
  api_post_json "/panel/api/inbounds/update/${id}" "$file" | jq .
}
...
write_trojan_per_domain() {
  if [ "${ALLOW_SELF_SIGNED_TLS:-0}" != "1" ] && { [ -z "${TLS_CERT_FILE:-}" ] || [ -z "${TLS_KEY_FILE:-}" ]; }; then
    echo "Skipping Trojan: set TLS_CERT_FILE/TLS_KEY_FILE or ALLOW_SELF_SIGNED_TLS=1."
    return
  fi

  local cert_file key_file clients_json spec domain port remark file client_json safe
  if [ -n "${TLS_CERT_FILE:-}" ] && [ -n "${TLS_KEY_FILE:-}" ]; then
    cert_file="$TLS_CERT_FILE"
    key_file="$TLS_KEY_FILE"
  else
  ...
  clients_json="$(build_trojan_domain_client_specs)"
  ...
  while IFS= read -r spec; do
    ...
    safe="$(safe_node_label "$domain")"
    [ -n "$safe" ] || safe="domain"
    remark="auto-trojan-ws-tls-${port}"
    file="runtime/trojan-ws-tls-${port}-${safe}.json"
    client_json="$(printf '%s' "$spec" | jq -s '.')"
    ...
    if add_inbound_if_missing "$remark" "$file"; then
      {
        ...
        printf '%s\n' "$spec" | jq -r \
          --arg remark "$remark" \
          '"trojan://\(.password)@\(.domain):\(.port)?type=ws&security=tls&sni=\(.domain)&path=%2Ftrojan#\($remark)@\(.domain)"'
        echo
      } >> runtime/client-links.txt
    else
      update_trojan_domain_clients "$remark" "$client_json" "$domain" || true
    fi
  done < <(printf '%s' "$clients_json" | jq -c '.[]')
}
